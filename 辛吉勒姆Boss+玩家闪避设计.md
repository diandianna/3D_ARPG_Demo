...
> 生成 2026-08-23。原则：**面试看重「讲得清」，不看重「写得全」**。demo 目标是能跑通核心闭环 + 讲清设计决策，不是复刻黑魂。
...
| **② 核心（demo 亮点，够面试）** | 极限闪避 → 子弹时间（复用 BulletTimeMgr） | 技能表（距离+CD+权重）+ 霸体/韧性 |
...
- 更硬核版（面试知识）：真正的 just-frame dodge 是"攻击前摇命中瞬间按键闪避"；demo 用"无敌帧内被攻击"版足够。
